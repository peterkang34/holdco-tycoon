...
| 13 | **`@vercel/kv` zrange members come back as STRING or OBJECT** | `if (typeof member !== 'string') continue; JSON.parse(member)` | Normalize both forms — use `parseLeaderboardMember()` (object → use as-is; string → parse) | Upstash auto-deserializes JSON-shaped sorted-set members on read. The scenario leaderboard/records/archive readers assumed strings → a stored entry counted toward `zcard` but was silently dropped from `entries` (board showed "1 entry" but rendered none). Global leaderboard (`get.ts:21`) always handled both; the scenario readers didn't. Tests only mocked string members, so it slipped through — always test the OBJECT-member path too. |
| 14 | **A store-action gate the UI eligibility check doesn't mirror = a button that silently no-ops** | UI renders a Forge/action button from an eligibility list; the store action has an EXTRA gate (achievement/mode) the list ignores | Route BOTH through one predicate (`isPlatformRecipeUnlocked`). Eligibility list excludes what the action would reject | `forgeIntegratedPlatform` gated `cross_saas_services_vertical` on `unlockedMechanics.crossSectorSaasServices`, but `checkPlatformEligibility` never checked it → the button rendered and the click did nothing. Scenario mode earns no achievements, so it failed forever ("tried 4-5 rounds, wouldn't take"). Scenario mode now SUSPENDS the gate (sealed sandbox — the scenario's mix is the authority, like `getAvailableSectors`). |
...
- `src/hooks/useUnlocks.ts` — achievement persistence + `shouldEarnAchievements({ isScenarioChallengeMode })`. **Scenario games earn NO global achievements** (sealed sandbox — rigged scenarios would otherwise be a backdoor for farming sector-gating achievements). Enforced in TWO places (a client gate alone is insufficient — the server recomputes achievements from `game_history`): (1) client write — `GameOverScreen` zeroes `earnedAchievements` in scenario mode; (2) server recompute — `api/_lib/achievementBackfill.ts` `evaluateGameAchievements` early-returns `[]` when `game.scenario_challenge_id != null`, which also neutralizes legacy scenario rows. Symmetric with suspending sector unlock gates for scenario availability (`getAvailableSectors` `isScenario`).
...
- `src/data/sectors.ts` — `getAvailableSectors(isFamilyOffice, unlockedSectorIds, isChallenge, isScenario)`. **Scenarios suspend achievement unlock gates:** `isScenario=true` returns `SECTOR_LIST_SCENARIO` (all sectors except FO-exclusive proSports), so an admin's `allowedSectors` can include normally-gated sectors (fintech, aerospace, privateCredit, mediaEntertainment). The scenario's own `allowedSectors` is the authority, not the player's unlocks. Deal-gen threads `isScenario` from the scenario call sites in `useGame.ts` through `generateDealPipeline` (`businesses.ts`). Peer challenges (`isChallenge` without `isScenario`) still use the gate-respecting standard list for fairness. **Two systems gate gated sectors** — (1) the availability POOL (`getAvailableSectors`, fixed by `isScenario`), and (2) the deal-flow WEIGHTING (`getSectorWeightsForRound`/`pickWeightedSector`, gated by `unlockedSectorIds` membership). Both must be opened or gated sectors appear but are under-weighted: scenario call sites pass `UNLOCKABLE_SECTOR_IDS` as `unlockedSectorIds` so gated sectors get premium-tier weight, not just variety-step reachability.
...
- `src/components/screens/ScenariosScreen.tsx` — **Player-facing Scenarios landing page** (App `Screen='scenarios'`, `?tab=scenarios`): Live Now + Past Challenges, Play CTA, inline scoreboard (reuses exported `ScenarioDetail`), "Your best: #rank". Flag-gated; reachable from the home mode tile. `src/utils/scenarioCountdown.ts` = shared countdown/end-date helpers.
- **Account-required-to-play** (Scenario Challenges): gate at the Play-card click + `ScenarioSetupView.onEnter` (anonymous → `openScenarioAccountWall` in `useAuth`; the `AccountModal` sets `redirectTo=/?se={id}` so sign-in resumes into the scenario — App's auth-callback cleanup preserves the query string). Server enforcement is the 401 in `api/scenario-challenges/submit.ts` (`!verifiedPlayerId`). Admin `#/se-preview` is exempt.
- `api/player/scenario-records.ts` — per-account scenario record (attempts/best/rank per scenario; 401 for anon). KV-scans each scenario's sorted set for the player's rank. Surfaced in `StatsModal` ("My Scenario Record") + the landing-page cards. `api/player/history.ts` selects `scenario_challenge_id` so the UI can label scenario rows.
- **Migrations**: `006-earned-achievement-ids.sql` (latent missing-column fix), `007-scenario-records-index.sql` (per-player scenario query index). Both idempotent; require `005` applied first.
