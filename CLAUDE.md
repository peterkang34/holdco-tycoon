...
- **Tests**: Vitest in `src/engine/__tests__/` — 664 tests across 13 suites (incl. display-proofreader)
...
- `src/engine/__tests__/display-proofreader.test.ts` — 102 tests: UI copy vs engine constants (MUST update when changing mechanics or UI copy)
...
- `src/data/platformRecipes.ts` — 35 integrated platform recipes (29 within-sector + 6 cross-sector)
...
- **`display-proofreader.test.ts`** — 102 tests that validate UI copy matches engine constants
