...
- **State**: Zustand store in `src/hooks/useGame.ts`, persisted as `holdco-tycoon-save-v34`
- **Tests**: Vitest in `src/engine/__tests__/` — 1390 tests across 24 suites (incl. display-proofreader + playtest system)
...
- `src/engine/__tests__/display-proofreader.test.ts` — 261 tests: UI copy vs engine constants (MUST update when changing mechanics or UI copy)
...
- **`display-proofreader.test.ts`** — 261 tests that validate UI copy matches engine constants
...
- **Save migrations**: Always back-fill new fields with sensible defaults; use `sharesOutstanding || 1` for division safety. Current: v34
...
- **Portfolio complexity cost**: Activates at 5 businesses (4 in quick mode). MAX_ACTIVE_SHARED_SERVICES = 3 (not 5). Each active SS offsets 1/3 of cost. Integrated platform constituents count as 1 entity. Cost deducted in `advanceToEvent()` waterfall, displayed in CollectPhase
